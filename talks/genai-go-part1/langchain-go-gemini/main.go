...
// availableTools simulates the tools/functions we are making available for
// the model. Can be easily replaced with actual API calls.
...
func main() {
	...
	llm, err := googleai.New(ctx, googleai.WithAPIKey(genaiKey), googleai.WithDefaultModel("gemini-1.5-pro"))
	if err != nil {
		log.Fatal(err)
	}
